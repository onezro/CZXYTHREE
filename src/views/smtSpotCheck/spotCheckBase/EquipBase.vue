            ...
            <el-row :gutter="10">
                ...
                <el-col :span="16">
                    ...
                    <el-table v-else :data="currentSubItems" border size="small" style="width: 100%"
                        ...
                        <!-- 新增执行频率列 -->
                        <el-table-column :label="t('smtSpotCheck.firstBase.subItemStatus') || '执行频率'" min-width="120">
                            <template #default="{ row }">
        ...
        <el-dialog :title="$t('publicText.add')" :lock-scroll="false" v-model="addDialogVisible" width="80%" @close="addDialogCancel"
            ...
            <el-form :model="addForm" ref="addFormRef" label-width="120px" :rules="formRules">
                ...
                <el-table :data="addForm.StepItemList" border size="small" style="width: 100%" height="400">
                    ...
                    <!-- 新增执行频率下拉框 -->
                    <el-table-column :label="t('smtSpotCheck.firstBase.subItemStatus') || '执行频率'" min-width="140">
                        <template #default="{ row }">
                            <el-select v-model="row.SubItemStatus" size="small" style="width: 100%">
                                <el-option
...
import { GetEquipInspectData, InserEquipInspectData, UpdateEquipInspectData, DeleteEquipInspectData } from "@/api/smtSpotCheck/base/index"
...
const statusOptions = ref([
    { label: "白班，夜班执行", value: "S" },
    { label: "每天执行一次", value: "D" },
    { label: "每周执行一次", value: "W" },
    { label: "每月执行一次", value: "M" },
    { label: "每季执行一次", value: "Q" },
    { label: "每年执行一次", value: "Y" },
]);
...
const getSubItemsByStep = (step: number) => {
    return rawFlatData.value.filter(item => item.Step === step).map(item => ({
        ...
        SubItemStatus: item.SubItemStatus || "D",  // 默认值 D
...
// 获取数据
const getData = () => {
    GetEquipInspectData({}).then((res: any) => {
        if (res.Success) {
...
const deleteSubItem = async (index: number) => {
    ...
    try {
        ...
        const newSubItems = subItems.filter((_, idx) => idx !== index).map(item => ({
            ...
            SubItemSolution: item.SubItemSolution || "",
            SubItemAim: item.SubItemAim || "",
            SubItemStatus: item.SubItemStatus || "D",
        }));
        // 调用更新接口提交完整的工序信息
        const requestData = {
            Product: "",
...
const submitAdd = () => {
    addFormRef.value.validate(async (valid: boolean) => {
        ...
        for (let i = 0; i < addForm.StepItemList.length; i++) {
            ...
            if (!item.SubItemStatus) {
                ElMessage.warning(`请选择第${i+1}项的执行频率`);
                return;
            }
        }
        ...
        try {
            const res: any = await InserEquipInspectData(requestData);
            if (res.Success) {
