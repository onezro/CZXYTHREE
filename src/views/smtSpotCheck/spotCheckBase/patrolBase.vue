...
import { GetPatrolInspectData, InserPatrolInspectData, UpdatePatrolInspectData, DeletePatrolInspectData } from "@/api/smtSpotCheck/base/index"
...
// 获取数据
const getData = () => {
    GetPatrolInspectData({}).then((res: any) => {
        if (res.Success) {
...
const handleDelete = (row: any) => {
    ElMessageBox.confirm(`${t('publicText.confirmDelete')}【${row.Step}】?`, t("publicText.confirm"), {
        ...
        DeletePatrolInspectData({ Step: row.Step }).then((res: any) => {
            if (res.Success) {
                ElMessage.success(res.Msg || "删除成功");
                getData();
            } else {
...
const submitAdd = () => {
    addFormRef.value.validate(async (valid: boolean) => {
        ...
        try {
            const res: any = await InserPatrolInspectData(requestData);
            if (res.Success) {
...
const submitEdit = () => {
    editFormRef.value.validate(async (valid: boolean) => {
        ...
        try {
            const res: any = await UpdatePatrolInspectData(requestData);
            if (res.Success) {
