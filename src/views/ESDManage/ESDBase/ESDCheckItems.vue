    ...
    <el-card shadow="always" :body-style="{ padding: '8px' }">
      <div class="flex justify-between items-center">
        ...
        <el-form ref="queryFormRef" :inline="true" :model="queryParams" label-width="auto" @submit.prevent>
          <el-form-item :label="t('esd.checkContent.content')" class="mb-2">
            <el-input v-model="queryParams.CheckContent" :placeholder="t('esd.checkContent.contentPlaceholder')"
      ...
      <el-table :data="tableData" ref="tableRef" border :height="tableHeight" style="width: 100%" stripe size="small"
        ...
        <el-table-column prop="UpdateUser" :label="t('esd.checkContent.updateUser')" :min-width="getColumnWidth('UpdateUser')" />
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm(
    `${t('publicText.confirm')}${t('publicText.delete')}【${row.CheckContent}】?`,
    t('publicText.tip'),
    ...
    .then(async () => {
      ...
      try {
        const currentUser = userStore.getUserInfo || 'system';
        let data= { Id: row.Id, UserName: currentUser };
        const res: any = await DeleteCheckContent(data);
        if (res.Success) {
