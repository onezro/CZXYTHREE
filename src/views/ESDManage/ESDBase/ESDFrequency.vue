    ...
    <el-card shadow="always" :body-style="{ padding: '8px' }">
      <div class="flex justify-between items-center">
        ...
        <el-form ref="queryFormRef" :inline="true" :model="queryParams" label-width="auto" @submit.prevent>
          <el-form-item :label="t('esd.checkFrequency.frequency')" class="mb-2">
            <el-input v-model="queryParams.CheckFrequency" :placeholder="t('esd.checkFrequency.frequencyPlaceholder')"
      ...
      <el-table :data="tableData" ref="tableRef" border :height="tableHeight" style="width: 100%" stripe size="small"
        ...
        <el-table-column prop="CheckFrequency" :label="t('esd.checkFrequency.frequency')"
          :min-width="getColumnWidth('CheckFrequency')" />
        <!-- <el-table-column prop="Status" :label="t('esd.checkFrequency.status')" :min-width="getColumnWidth('Status')">
          <template #default="{ row }">
            <el-tag :type="row.Status === 0 ? 'success' : 'danger'" size="small">
              {{ row.Status === 0 ? t('esd.checkFrequency.enable') : t('esd.checkFrequency.disable') }}
            </el-tag>
          </template>
        </el-table-column> -->
        <el-table-column prop="CreateTime" :label="t('esd.checkFrequency.createTime')"
          :min-width="getColumnWidth('CreateTime')" />
        <el-table-column prop="CreateUser" :label="t('esd.checkFrequency.createUser')"
          :min-width="getColumnWidth('CreateUser')" />
        <el-table-column prop="UpdateTime" :label="t('esd.checkFrequency.updateTime')"
          :min-width="getColumnWidth('UpdateTime')" />
        <el-table-column prop="UpdateUser" :label="t('esd.checkFrequency.updateUser')"
...
const openEdit = (row: any) => {
  isEdit.value = true;
  formData.Id = row.Id;
  formData.CheckFrequency = row.CheckFrequency;
  dialogVisible.value = true;
};
...
const submitForm = async () => {
  ...
  try {
    ...
    let res: any;
    if (isEdit.value) {
      res = await UpdateCheckFrequency({ Id: formData.Id, CheckFrequency: formData.CheckFrequency, UserName: currentUser });
      if (res.Success) ElMessage.success(t('message.editSuccess'));
    } else {
      res = await AddCheckFrequency({ CheckFrequency: formData.CheckFrequency, UserName: currentUser });
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm(
    `${t('publicText.confirm')}${t('publicText.delete')}【${row.CheckFrequency}】?`,
    t('publicText.tip'),
    ...
    .then(async () => {
      ...
      try {
        const currentUser = userStore.getUserInfo || 'system';
        const res: any = await DeleteCheckFrequency({ Id: row.Id, UpdateUser: currentUser });
        if (res.Success) {
