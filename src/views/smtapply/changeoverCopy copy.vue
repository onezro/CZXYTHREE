        ...
        <div class="main-content">
            ...
            <div class="info-panel">
                ...
                <div class="panel-body">
                    <el-form ref="formRef" :model="form" label-width="100px" class="info-form" label-position="left"
                        size="large">
                        ...
                        <div v-for="(item, index) in filteredDevices" :key="item.Equipid" class="device-card" :class="{
                            ...
                            <div class="device-content">
                                ...
                                <!-- <div class="device-details">
                                    ...
                                    <div class="detail-item" v-if="item.ConveryNum > 0 && item.ConveryNum != 0">
                                        <span class="detail-label">轨道1</span>
                                        <div class="detail-value">
                                            <span class="status-indicator"
                                                :class="getStatusClass(item.ConverConveyorStatus)"></span>
                                            <span :class="getTextColorClass(item.ConverConveyorStatus)">
                                                {{ getStatusText(item.ConverConveyorStatus) }}
                                            </span>
                                        </div>
                                    </div>
                                    <div class="detail-item" v-if="item.ConveryNum > 1 && item.ConveryNum != 0">
                                        <span class="detail-label">轨道2</span>
                                        <div class="detail-value">
                                            <span class="status-indicator"
                                                :class="getStatusClass(item.ConverConveyorStatus2)"></span>
                                            <span :class="getTextColorClass(item.ConverConveyorStatus2)">
                                                {{ getStatusText(item.ConverConveyorStatus2) }}
                                            </span>
                                        </div>
                                    </div>
                                    <div class="detail-item" v-if="item.ConveryNum > 2 && item.ConveryNum != 0">
                                        <span class="detail-label">轨道3</span>
                                        <div class="detail-value">
                                            <span class="status-indicator"
                                                :class="getStatusClass(item.ConverConveyorStatus3)"></span>
                                            <span :class="getTextColorClass(item.ConverConveyorStatus3)">
                                                {{ getStatusText(item.ConverConveyorStatus3) }}
                                            </span>
                                        </div>
                                    </div>
                                    <div class="detail-item" v-if="item.ConveryNum > 3 && item.ConveryNum != 0">
                                        <span class="detail-label">轨道4</span>
                                        <div class="detail-value">
                                            <span class="status-indicator"
                                                :class="getStatusClass(item.ConverConveyorStatus4)"></span>
                                            <span :class="getTextColorClass(item.ConverConveyorStatus4)">
                                                {{ getStatusText(item.ConverConveyorStatus4) }}
                                            </span>
                                        </div>
                                    </div>
                                </div> -->
                                <div class="device-details">
                                    ...
                                    <div class="detail-item" v-for="(status, idx) in item.conveyorStatusList"
                                        ...
                                        <div class="detail-value">
                                            ...
                                            <span :class="getTextColorClass(status)">{{ getStatusText(status) }}</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="device-actions flex justify-between items-center">
                                    ...
                                    <div>
                                        <el-tag size="large" :type="getDeviceStatusTextType(item)" effect="light" round>
                                            <span class="text-lg font-black">{{ getDeviceStatusText(item) }}</span>
                                        </el-tag>
...
const lineData = ref<any[]>([
    { Equipid: 1, EquipName: "Laser", McIdStatus: 1, ConveryNum: 1,conveyorStatusList:[] },
    { Equipid: 2, EquipName: "Printer", McIdStatus: 1, ConveryNum: 1, conveyorStatusList:[]},
    { Equipid: 3, EquipName: "SPI", McIdStatus: 1, ConveryNum: 1, conveyorStatusList:[]},
    { Equipid: 4, EquipName: "Mounter", McIdStatus: 1, ConveryNum: 1, conveyorStatusList:[]},
    { Equipid: 5, EquipName: "M-AOI", McIdStatus: 1, ConveryNum: 1, conveyorStatusList:[] },
    ...
    lineData.value = lineData.value.map((device) => {
        ...
        if (status) {
            ...
            for (let i = 1; i <= device.ConveryNum; i++) {
                ...
                const fieldName = i === 1 ? 'ConverConveyorStatus' : `ConverConveyorStatus${i}`;
...
const confirmChangeOver = async () => {
    ...
    SMTChangeOverByOneKey({ ...data, OperatorUser: userStore.getUserInfo }).then((res: any) => {
...
const handleChangeOverResponse = (res: any) => {
    ...
    lineData.value = lineData.value.map((device) => {
        ...
        const mcIdStatus = mainStatusItem ? (mainStatusItem.success ? '1' : '0') : '2';
...
const getDeviceStatus = (device: any) => {
    ...
    if (device.conveyorStatusList.some((status: any) => status === '0')) return 'failure';

    // 检查是否有状态为4（换线中）或其他自定义中间状态
    if (device.McIdStatus === '4') return 'warning';
    if (device.conveyorStatusList.some((status: any) => status === '4')) return 'warning';
    ...
    const allConveyorSuccess = device.conveyorStatusList.every((status: any) => status === '1'|| status === '-1'); // 允许轨道状态为成功或待换线
    if (device.McIdStatus === '1'||device.McIdStatus === '-1' && allConveyorSuccess) return 'success';
