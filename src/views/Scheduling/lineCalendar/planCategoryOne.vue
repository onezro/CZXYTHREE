    ...
    <div class="p-2">
        ...
        <el-dialog :title="t('publicText.add')" v-model="addVisible" width="500px" :close-on-click-modal="false"
            ...
            <el-form ref="addFormRef" :model="addForm" :rules="formRules" label-width="auto">
                <el-form-item :label="t('Scheduling.planCategoryOne.SelectType')" prop="SelectType">
                    <el-input v-model="addForm.SelectType" :placeholder="t('message.pleaseInput')+t('Scheduling.planCategoryOne.SelectType')" clearable />
                </el-form-item>
            </el-form>
            <template #footer>
        ...
        <el-dialog :title="t('publicText.edit')" v-model="editVisible" width="500px" :close-on-click-modal="false"
            ...
            <el-form ref="editFormRef" :model="editForm" :rules="formRules" label-width="auto" :size="'small'">
                <el-form-item :label="t('Scheduling.planCategoryOne.SelectType')" prop="SelectType">
                    <el-input v-model="editForm.SelectType" :placeholder="t('message.pleaseInput')+t('Scheduling.planCategoryOne.SelectType')" clearable />
                </el-form-item>
            </el-form>
            <template #footer>
                <div class="dialog-footer">
...
const formRules = reactive({
    SelectType: [
        { required: true, message: t('message.pleaseInput')+t('Scheduling.planCategoryOne.SelectType'), trigger: 'blur' },
    ]
});
const openAdd = () => {
...
const submitEdit = () => { 
    editFormRef.value.validate((valid: any) => {
        if (valid) {
            UpdateWorkLineCalendarSelectTypeLevelOne(editForm.value).then((res: any) => {
                if (res.Success) {
...
const handleDelete = (row: any) => {
      ElMessageBox.confirm(`${t('publicText.confirm')}${t("publicText.delete")}【${row.SelectType}】?`, t("publicText.confirm"), {
