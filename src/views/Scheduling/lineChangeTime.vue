...
const formRules = reactive({
    LineName: [
        {
            required: true,
            message: t("message.pleaseSelect") + t("Scheduling.lineChangeTime.lineName"),
            trigger: "change",
        },
    ],
    TimeLong: [
        {
            required: true,
            message: t("message.pleaseInput") + t("Scheduling.lineChangeTime.timeLong"),
            trigger: "blur",
        },
        {
            type: "number",
            min: 0,
            message: t("Scheduling.lineChangeTime.timeLongValid"),
            trigger: "blur",
        },
    ],
    Unit: [
        {
            required: true,
            message: t("message.pleaseSelect") + t("Scheduling.lineChangeTime.unit"),
            trigger: "change",
        },
    ],
});
...
const submitAdd = () => {
    addFormRef.value.validate((valid: boolean) => {
        if (valid) {
            // 检查线体是否已存在
            const existItem = tableData.value.find(item => item.LineName === addForm.LineName);
            if (existItem) {
                ElMessageBox.confirm(
                    t("Scheduling.lineChangeTime.lineExistConfirm", { lineName: addForm.LineName }),
                    t("publicText.tip"),
                    {
...
const doSave = () => {
    ...
    InsertUpdateSwitchLineTime(params)
        .then((res: any) => {
            if (res.Success) {
...
const handleDelete = (row: any) => {
    ElMessageBox.confirm(
        `${t("publicText.confirm")}${t("publicText.delete")}【${row.LineName}】?`,
        t("publicText.tip"),
        {
            confirmButtonText: t("publicText.confirm"),
            cancelButtonText: t("publicText.cancel"),
