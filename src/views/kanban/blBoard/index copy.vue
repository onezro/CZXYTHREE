        ...
        <el-row :gutter="6">
          <el-col :span="8">
            <div class="card-container">
              <div class="card-header"><i class="fa fa-tasks"></i>过程PMM2026年度目标与实际统计（1-5月份）</div>
              <ProcessPpmChart class="chart-container" :months="ppmMonths" :targetValue="ppmTarget"
                :actualData="ppmActual" />
            </div>
          </el-col>
          <el-col :span="8">
            <div class="card-container">
              <div class="card-header"><i class="fa fa-tasks"></i>SMT设备开线情况</div>
              <SmtPieChart class="chart-container" :production="smtData.production" :maintenance="smtData.maintenance"
                :fault="smtData.fault" />
            </div>
          </el-col>
          <el-col :span="8">
            <div class="card-container">
              <div class="card-header"><i class="fa fa-tasks"></i>组织车间设备开线情况</div>
              <OrgPieChart class="chart-container" :production="orgData.production" :maintenance="orgData.maintenance"
                :fault="orgData.fault" />
            </div>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="6">
          ...
          <el-col :span="8">
            <div class="card-container">
              ...
              <!-- <MttrGaugeChart class="chart-container" :value="900" :max="720" targetText="" unit="h" /> -->
              <MttrGaugeChart class="chart-container" :value="720" :max="700" targetText="" unit="h" />
            </div>
          </el-col>
          <el-col :span="8">
            <div class="card-container">
              ...
              <MbtfGaugeChart class="chart-container" :value="mttrValue" :max="60" :threshold="30" targetText=""
                unit="min" />
              <!-- <MbtfGaugeChart class="chart-container" :value="mttrValue" :max="30" targetText="" unit="min" /> -->
            </div>
          </el-col>
        </el-row>
      </div>
...
import ProcessPpmChart from './components/ProcessPpmChart.vue'
import SmtPieChart from './components/SmtPieChart.vue'
import OrgPieChart from './components/OrgPieChart.vue'
...
// 过程 PPM 数据
const ppmMonths = ['1月', '2月', '3月', '4月', '5月']
const ppmTarget = 20
const ppmActual = ref(
  Array.from({ length: 5 }, () => Math.floor(Math.random() * 4) + 12)
)
...
onMounted(() => {
  ...
  fluctuationTimer.value = setInterval(() => {
    mtbfValue.value += Math.random() < 0.5 ? -1 : 1
    mttrValue.value += Math.random() < 0.5 ? -1 : 1
    // 限制范围（可选）
    if (mtbfValue.value < 719) mtbfValue.value = 719
    if (mtbfValue.value > 721) mtbfValue.value = 721
