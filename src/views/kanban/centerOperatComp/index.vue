...
// ---------- 重置标记（用于每日7:00重置）----------
// 记录上一次执行重置的日期（YYYY-MM-DD），防止同一分钟内多次重置
...
/** 检查并执行每日7:00重置 */
const checkAndResetMonthlyActual = (now: Date) => {
  const today = formatDateStr(now)
  // 如果今天已经重置过，跳过
  if (lastResetDay === today) return

  // 如果当前时间已经过了当天的7:00，执行重置
  if (now.getHours() >= 7) {
...
const updateTime = () => {
  ...
  // 更新安全生产天数
  safeDays.value =
    Math.floor((now.getTime() - new Date('2026-01-28').getTime()) / (1000 * 60 * 60 * 24)) + 1

  // 检查是否到达每日7:00重置点
  checkAndResetMonthlyActual(now)
}
...
onMounted(() => {
  // 页面首次加载时，如果当前时间已经过了今天7:00，立刻重置一次（保证重启后也从339416开始）
  checkAndResetMonthlyActual(new Date())
